...
int main() {
   ...
   ull smallSize = 100000;
   list<Folder*> smallFolders;
   ull smallFoldersSize = 0;
   list<Folder*> fifo;
   ...
   while (!fifo.empty()) {
      ...
      if (currentFolder->size() <= smallSize) {
         smallFolders.push_back(currentFolder);
      }
   ...
   for (Folder* smallFolder : smallFolders) {
      smallFoldersSize += smallFolder->size();
   }

   cout << "Combined size of folder with size <= " << smallSize << ": " << smallFoldersSize;
}
